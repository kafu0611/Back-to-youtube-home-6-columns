...
哪张卡”，`MutationObserver` 负责在无限滚动、SPA 导航后重新登记卡片。YouTube 复用
卡片时可能只改链接的 `href` 或文本节点内容，所以新增节点、`href` 变化和文本变化
都会被监听；登记只针对本次真正变化的那张卡片或那棵子树，并按帧合并，因此页面上
无关的高频 DOM 变动不会引发全页重扫。日期通过
...
- `0.5`：精简约三分之一代码；样式改为尽早注入，避免列数闪烁；卡片登记改为按变化
  子树增量处理，被 YouTube 回收复用的卡片也能刷新日期。
- `0.4`：新增精确发布日期。
