...
布局样式由 `<html>` 上的 `data-ytg-home` 开关控制，只在主页生效，
离开主页时会自动失效。

日期功能只处理进入视口的普通视频卡片：`IntersectionObserver` 负责判断“该取
哪张卡”，`MutationObserver` 负责登记无限滚动时新增的卡片，SPA 导航完成后
脚本会重新检查当前页面。日期通过 YouTube 页面自带的 Innertube player 接口
读取，并在当前页面内按视频 ID 缓存，同一个视频不会重复请求。替换时
原来的相对时间会存进 `title`，鼠标悬停仍可看到。
